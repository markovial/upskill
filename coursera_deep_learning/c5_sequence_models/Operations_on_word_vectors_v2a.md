...
def neutralize(word, g, word_to_vec_map):
    ...
    e_biascomponent =  ((e @ g) / np.sqrt(np.sum(g**2))**2) * g
...
def equalize(pair, bias_axis, word_to_vec_map):
    ...
    mu_B = ((mu @ bias_axis) / np.sqrt(np.sum(bias_axis**2))**2) * bias_axis
    mu_orth = mu - mu_B

    # Step 4: Use equations (7) and (8) to compute e_w1B and e_w2B (≈2 lines)
    e_w1B = ((e_w1 @ bias_axis) / np.sqrt(np.sum(bias_axis**2))**2) * bias_axis
    e_w2B = ((e_w2 @ bias_axis) / np.sqrt(np.sum(bias_axis**2))**2) * bias_axis
