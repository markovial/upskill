...
    Returns:
    ...
    #one_hot = tf.reshape( tf.one_hot(label, depth , axis = 0 ) , ( 1, depth))
...
def compute_cost(logits, labels):
    ...
    # cost = ...
    # YOUR CODE STARTS HERE
    # sum (yhat - y) = sum ( y_pred - y_true )
    
    # logits.shape ( num_categories , num_examples )
    # labels.shape ( num_categories , num_examples )
    # desired for tf.keras.losses.categorical_crossentropy :
    # logits.shape ( num_examples , num_categories )
    # labels.shape ( num_examples , num_categories )
    
    cost = tf.keras.metrics.categorical_crossentropy(
                y_true      = tf.transpose(labels) ,
                y_pred      = tf.transpose(logits) ,
                from_logits = True
           )
    #cost = tf.keras.metrics.categorical_crossentropy( labels ,  logits )
